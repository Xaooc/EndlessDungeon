...
class Events:
    ...
    async def get_result(self, event: int, con: int, dex: int, mnd: int) -> dict:
        mod = 0
        if self.events.get(event).get('type') == 'con':
            mod = (con - 10) // 2
        if self.events.get(event).get('type') == 'dex':
            mod = (dex - 10) // 2
        if self.events.get(event).get('type') == 'mnd':
            mod = (mnd - 10) // 2
        roll = randint(1, 20) + mod
        # получаем результат броска
        fin = roll > self.events.get(event).get('difficulty')
        # выбираем случайный результат в зависимости от броска
        result = choice(list(self.events.get(event).get('result')[fin].keys()))
        # выбираем случайное описание полученного результата
        description = choice(self.events.get(event).get('result')[fin].get(result))
        time = self.events.get(event).get('time')
        info_result = Result(self.events.get(event).get('difficulty'), result)
        info = await info_result.get_result()
        return {'result': result, 'description': description+info.get('description'), 'time': time,
                'effect': info.get('effect'), 'type': info.get('type')}

    async def get_button(self, events: list) -> list:
        return [self.events[i].get('move') for i in events]
